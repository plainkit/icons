package lucide

import x "github.com/bloxui/blox"

// Anvil creates an Anvil Lucide icon.
func Anvil(args ...x.SvgArg) x.Component {
	svgArgs := buildLucideArgs("lucide lucide-anvil", args)
	svgArgs = append(svgArgs,
		x.Child(x.Path(x.D("M7 10H6a4 4 0 0 1-4-4 1 1 0 0 1 1-1h4"))),
		x.Child(x.Path(x.D("M7 5a1 1 0 0 1 1-1h13a1 1 0 0 1 1 1 7 7 0 0 1-7 7H8a1 1 0 0 1-1-1z"))),
		x.Child(x.Path(x.D("M9 12v5"))),
		x.Child(x.Path(x.D("M15 12v5"))),
		x.Child(x.Path(x.D("M5 20a3 3 0 0 1 3-3h8a3 3 0 0 1 3 3 1 1 0 0 1-1 1H6a1 1 0 0 1-1-1"))),
	)
	return x.Svg(svgArgs...)
}
