package lucide

import x "github.com/bloxui/blox"

// FileSliders creates a File Sliders Lucide icon.
func FileSliders(args ...x.SvgArg) x.Component {
	svgArgs := buildLucideArgs("lucide lucide-file-sliders", args)
	svgArgs = append(svgArgs,
		x.Child(x.Path(x.D("M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7Z"))),
		x.Child(x.Path(x.D("M14 2v4a2 2 0 0 0 2 2h4"))),
		x.Child(x.Path(x.D("M8 12h8"))),
		x.Child(x.Path(x.D("M10 11v2"))),
		x.Child(x.Path(x.D("M8 17h8"))),
		x.Child(x.Path(x.D("M14 16v2"))),
	)
	return x.Svg(svgArgs...)
}
